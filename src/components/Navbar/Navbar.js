...
const Navbar = () => {
  ...
  const handleClick = () => {
    setShow(true); // Show the registration form when the button is clicked
  };

  const handleClose = () => {
    setShow(false); // Hide the registration form when needed
  };
    ...
    <nav className="flex items-center justify-between p-4 shadow-md">
      ...
      <div className="flex items-center space-x-4">
        ...
        <div className="navbar-buttons space-x-4">
          ...
          <button className="signup-btn bg-green-500 text-white px-4 py-2 rounded hover:bg-green-600" onClick={handleClick}>
            Register
          </button>
        </div>
      </div>

      {/* Conditionally render the RegistrationForm centered */}
      {show && (
